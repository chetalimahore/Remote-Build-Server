...
namespace MessagePassingComm
{
    public class Receiver : IMessagePassingComm
    {
        ...
        public bool openFileForWrite(string name, string dest)
        {
            ServiceEnvironment.fileStorage = dest;
            Console.WriteLine("\n Writing file " + name + " to Destination path:" + dest + " \n");
            try
            {
    ...
    public class Sender
    {
        ...
        public bool postFile(string fileName, string dest)
        {
            FileStream fs = null;
            long bytesRemaining;
            // Console.WriteLine("Inside postfile, destination =" + dest);
            try
            {
                string path = Path.Combine(ClientEnvironment.fileStorage, fileName);
                Console.WriteLine("\n Source path of the file - " + fileName + " is " + path + "\n");
                fs = File.OpenRead(path);
                bytesRemaining = fs.Length;
                channel.openFileForWrite(fileName, dest);
                while (true)
                {
    ...
    class TestPCommService
    {
        ...
        public static bool testComm(string dest)
        {
            bool test = true;
            ...
            foreach (string name in names)
            {
                ...
                bool transferSuccess = comm.postFile(name, dest);
        ...
        public class MPCommService
        {
            ...
            public static void Main(string[] args)
            {
                ...
                TestUtilities.checkResult(testComm("../../../Repository/RepoStorage/All_Files"), "Comm");
                TestUtilities.putLine();
